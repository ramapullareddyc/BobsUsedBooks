...
namespace Bookstore.Data.Repositories
{
    public class AddressRepository : IAddressRepository
    {
        ...
        {
            var address = await dbContext.Set<Address>().SingleOrDefaultAsync(x => x.Customer.Sub == sub && x.Id == id);
        ...
        {
            return await dbContext.Set<Address>().SingleOrDefaultAsync(x => x.Customer.Sub == sub && x.Id == id && x.IsActive == true);
        }

        async Task<IEnumerable<Address>> IAddressRepository.ListAsync(string sub)
        {
            return await dbContext.Set<Address>().Where(x => x.Customer.Sub == sub && x.IsActive == true).ToListAsync();
        }
